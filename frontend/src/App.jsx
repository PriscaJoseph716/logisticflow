...
const translations = {
  en: {
    ...
    reports: {
      ...
      customerReportSection: "Individual customer report",
      customerReportText: "Download one customer PDF with shipments, deliveries, and billing.",
      selectCustomer: "Select customer",
      downloadCustomerReport: "Download customer report",
      chooseCustomer: "Choose a customer first.",
      ...
      page: "Page",
      of: "of",
      companyWebsite: "logisticsflow.app",
      partial: "Partial",
      overdue: "Overdue",
      inProgress: "In progress",
      code: "Code",
  ...
  sw: {
    ...
    reports: {
      ...
      customerReportTitle: "Ripoti ya Mteja",
      customerReportSection: "Ripoti ya mteja mmoja",
      customerReportText: "Pakua PDF ya mteja mmoja yenye mizigo, uwasilishaji, na bili.",
      selectCustomer: "Chagua mteja",
      downloadCustomerReport: "Pakua ripoti ya mteja",
      chooseCustomer: "Chagua mteja kwanza.",
      shipmentsSection: "Mizigo",
      deliveriesSection: "Uwasilishaji",
      billingSection: "Malipo",
      ...
      shipmentNo: "Namba ya Mzigo",
      deliveryNo: "Namba ya Uwasilishaji",
      vehicleNo: "Namba ya Gari",
      supplierNo: "Namba ya Msambazaji",
      recordNo: "Namba ya Rekodi",
...
function App() {
  ...
  const reportPreview = useMemo(() => {
    ...
    const columnLabels = {
      ...
      shipmentNo: t.reports.shipmentNo,
      deliveryNo: t.reports.deliveryNo,
      vehicleNo: t.reports.vehicleNo,
      supplierNo: t.reports.supplierNo,
      recordNo: t.reports.recordNo,
      ...
      partial: t.reports.partial,
      overdue: t.reports.overdue,
      inProgress: t.reports.inProgress,
  ...
  const handleModalSave = () => {
    ...
    if (modal.type === "shipment") {
      ...
      let maxShipmentSeq = appData.shipments.length;
      for (const shipment of appData.shipments) {
        const match = String(shipment.shipmentCode || "").match(/(?:SH|SHP)-?(\d+)/i);
        if (match) maxShipmentSeq = Math.max(maxShipmentSeq, Number(match[1]));
      }
      const nextShipmentCode = `SH-${String(maxShipmentSeq + 1).padStart(4, "0")}`;
      softPersist(
        () =>
          shipmentsApi.create({
  ...
  const exportSectorReport = (format) => {
    ...
    const labels = {
      ...
      all: t.common.all,
      poweredBy: "Generated by LogisticsFlow",
      ...
      summary: {
        ...
        totalShipments: t.reports.totalShipments,
        totalQuantity: t.reports.totalQuantity,
      },
    };
  ...
  const exportCustomerReport = (customerOverride = null, useAllPeriod = false) => {
    const customer =
      customerOverride ||
      appData.customers.find((item) => item.entityId === reportCustomerId || item.id === reportCustomerId);

    if (!customer) {
      showToast(t.reports.chooseCustomer, "error");
      return;
    }

    const filter = useAllPeriod ? "all" : reportFilter;
    const customRange = { from: reportCustomFrom, to: reportCustomTo };
    const inRange = useAllPeriod
      ? () => true
      : (dateValue) => isDateInRange(dateValue, filter, "", customRange);
    ...
    const file = buildCustomerReportPdf({
      customer,
      appData,
      labels,
      business: authSession?.business,
  ...
  const renderReports = () => (
    <div className="page-stack">
      ...
      <section className="glass-card table-card" style={{ padding: "20px 22px" }}>
        <div className="section-row">
          <div>
            <h3>{t.reports.customerReportSection}</h3>
            <p>{t.reports.customerReportText}</p>
          </div>
        </div>
        <div className="report-toolbar" style={{ marginTop: 16, flexWrap: "wrap" }}>
          <label className="language-picker compact" style={{ minWidth: 220 }}>
            <span>{t.reports.selectCustomer}</span>
            <select
            ...
            >
              ...
              {appData.customers.map((customer) => (
                <option key={customer.entityId || customer.id} value={customer.entityId || customer.id}>
                  {customer.name}
                </option>
              ))}
            </select>
          ...
          <button
            ...
            disabled={!appData.customers.length || !reportCustomerId}
          >
            <FileDown size={14} />
            {t.reports.downloadCustomerReport}
          </button>
        </div>
      </section>
